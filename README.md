...
| 证据域 | fact 实测事实 / case 案例（发生过的事，规则引用它作证据）|

> 证据域是 2026-09-19 扩的：项目实际长出了"实测数值 / 资产记录 / 事故案例"这类节点，
> 而它们**不能被已有类型覆盖而不丢区分**（详见 SKILL.md 的判据）。
> 同一次整理还**归并**了 8 种只有 1~2 条、明显是近义写法的类型（todo / lesson / fix / …）。

### 图库体检：`doctor`

```
$PY runner.py doctor        # 有问题 exit 1，可直接当门禁
```

四项：**枚举完整性** / 类型越界 / 重边 / id 污染。
存在的理由是一个真实事故：某项目图库 422 个节点里有 **163 个（39%）技能根本枚举不到** ——
`runner.py add` 会自动连根，而 `graph_crud bulk` 不会，且**没有任何一处报错**。
